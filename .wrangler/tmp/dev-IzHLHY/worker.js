...
// Dynamically build page titles & empty icons
...
async function loadItems(){
  ...
  try{
    ...
    updateCounts();loadStats();renderItems(allItems);
  }catch(e){
...
async function loadStats(){
  try{
    const res = await fetch(API.replace('/items','/stats'));
...
function renderItems(items){
  ...
  area.innerHTML='<div class="grid">'+items.map(item=>{
    ...
    return '<div class="card '+item.category+'" onclick="viewDetail(\\x27'+item.id+'\\x27)">'+preview+
      ...
      '<span class="card-category '+item.category+'">'+label+'</span></div>'+
...
function showModal(title,mode,item){
  ...
  const catOptions = CATEGORY_LIST.map(c =>
    '<option value="'+c.id+'"'+(selCat===c.id?" selected":"")+'>'+c.icon+' '+c.label+'</option>'
  ).join("");
...
function estimateSize(item) {
  ...
  if (item.fileData) {
    ...
    if (comma > 0) {
      size = Math.round((item.fileData.length - comma - 1) * 0.75);
    }
  }
  if (item.content) {
    ...
    if (match) {
      ...
      if (request.method === "PUT") {
        try {
          ...
          const newCategory = body.category && CATEGORIES.includes(body.category) ? body.category : existing.category;
          const updated = {
            ...existing,
          ...
          if (found.key !== newKey) await env.PERSONAL_CLOUD_KV.delete(found.key);
...
// .wrangler/tmp/bundle-QuT4ug/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-QuT4ug/middleware-loader.entry.ts
