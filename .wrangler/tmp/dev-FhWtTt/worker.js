...
function renderItems(items){
  ...
  area.innerHTML='<div class="grid">'+items.map(item=>{
    ...
    const preview=item.hasFile&&item.fileType&&item.fileType.startsWith("image/")?'<img src="/api/items/'+item.id+'/download?t='+AUTH_TOKEN+'"' style="width:100%;max-height:160px;object-fit:cover;border-radius:var(--radius-sm);margin-bottom:10px">'
...
function viewDetail(id){
  ...
  if(item.fileType&&item.fileType.startsWith("image/")){
    contentHtml='<img src="/api/items/'+item.id+'/download?t='+AUTH_TOKEN+'"' style="max-width:100%;border-radius:var(--radius-sm)">';
...
// .wrangler/tmp/bundle-Ak6uIR/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-Ak6uIR/middleware-loader.entry.ts
